-- UPC-level Easter analysis + post-Easter price variation
WITH src AS (
  SELECT
...
posteaster AS (
  SELECT COUNTRY_CODE, UPC_NBR, DEPT_CAT_NBR_DESC_COMB, SBU,
...
lifts AS (
  SELECT
    e.COUNTRY_CODE, e.UPC_NBR, e.UPC_DESC,
    e.DEPT_CAT_NBR_DESC_COMB AS CATEGORY, e.SBU,
  ...
  LEFT JOIN posteaster p USING (COUNTRY_CODE, UPC_NBR, DEPT_CAT_NBR_DESC_COMB, SBU)
)
SELECT * FROM lifts
