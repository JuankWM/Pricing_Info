...
<title>Easter Seasonality Analysis v2 2025 - Pricing Intelligence CAM</title>
...
def js_block(data_js: str) -> str:  # noqa: C901
    # NOTE: All emoji / special chars use plain JS strings or HTML entities.
    # Surrogate-pair escapes (\uD8xx) are intentionally avoided to prevent
    # UTF-8 encoding errors when writing the HTML file.
    return (
        "const D = " + data_js + ";\n"
        """
...
const FLAG={CAM:'[CAM]',CR:'[CR]',GT:'[GT]',HN:'[HN]',NI:'[NI]'};
...
function buildKPIs(){
  ...
  document.getElementById('kpiStrip').innerHTML=Object.entries(cs).map(([c,v])=>{
    ...
    const topLabel=top?top.cat.replace(/^[0-9]+ - /,'').replace(/^[0-9]+-/,'').slice(0,28):'\u2014';
    ...
    const subNote=isCAM?'<div class="text-xs text-blue-300 mt-0.5">USD &middot; agregado</div>':'';
    return '<div class="card kpi p-3 border-t-4 '+border+'">'
      +'<div class="text-xs text-gray-400 font-bold uppercase tracking-wide">'+CTRY_FULL[c]+'</div>'
      +subNote
      +'<div class="flex items-baseline gap-2 mt-0.5">'
      +'<span class="text-xl font-black '+numClr+'">'+v.total+'</span>'
      +'<span class="text-xs text-gray-400">categor\u00edas</span></div>'
      +'<div class="mt-1"><span class="font-bold text-green-700">'+v.seasonal+'</span>'
      +'<span class="text-gray-500 text-xs"> estacionales</span>'
      +'<span class="ml-1 text-xs font-bold bg-green-100 text-green-800 px-1.5 py-0.5 rounded-full">'+pct+'%</span></div>'
      +'<div class="text-xs text-gray-400 mt-1">'+v.wm_gain+' WM Gain &middot; '+v.price_war+' Price War</div>'
      +'<div class="text-xs text-blue-600 font-semibold mt-1 truncate" title="'+(top?top.cat:'')+'">'
      +'#1 '+topLabel+' '+topLift+'</div>'
      +'</div>';
  }).join('');
}
...
function buildInsights(){
  ...
  const tableRows=Object.entries(cs).map(([c,v])=>{
    ...
    return '<tr class="'+rowCls+'">'
      +'<td class="py-1.5 px-2 font-bold">'+CTRY_FULL[c]+'</td>'
      ...
      +'<td class="py-1.5 px-2"><div class="flex items-center gap-2">'
      +'<div class="flex-1 bg-gray-100 rounded-full h-2" style="min-width:80px">'
      +'<div class="'+(isCAM?'bg-blue-500':'bg-green-500')+' h-2 rounded-full" style="width:'+bar+'%"></div></div>'
      +'<span class="text-xs font-bold '+(isCAM?'text-blue-700':'text-green-700')+' w-10">'+pct+'%</span></div></td>'
  ...
  const bullets=[
    'Top Destination: '+topDest.map(r=>'<strong>['+r.c+'] '+r.cat.replace(/^[0-9]+ - /,'').replace(/^[0-9]+-/,'').slice(0,30)+' ('+r.wl.toFixed(1)+'x)</strong>').join(' &middot; ')+'.',
    '<strong>'+s.mkt_wide+' categor\u00edas</strong> con crecimiento total de mercado. <strong>'+s.wm_gain+'</strong> con ganancia de share exclusiva de Walmart.',
    ...
    'Monitorear categor\u00edas con <em>Stays Elevated</em> + <em>Seasonal=Yes</em>: precio alto post-Easter puede comprometer competitividad.',
  ];
...
function postBadge(b){
  const MAP={
    'Recovers to Base':'bg-rec|v Recovers',
    'Stays Elevated':'bg-elev|^ Elevated',
    'Stable Post-Easter':'bg-stab|= Stable',
    'Price Drop':'bg-sdrop|v Drop',
    'No Post Data':'bg-nodat|n/a',
  };
  const v=MAP[b]||'bg-nodat|n/a';
  const [cls,label]=v.split('|');
...
function renderCatTable(){
  ...
  document.getElementById('catTbody').innerHTML=rows.map(r=>
...
function renderUPCTable(){
  ...
  document.getElementById('upcTbody').innerHTML=rows.slice(0,300).map(r=>
    '<tr class="border-b border-gray-50 hover:bg-blue-50">'
    +'<td class="py-1 px-2 font-bold">'+r.c+'</td>'
...
function buildTop15(){
  ...
  charts.top15=new Chart(document.getElementById('chartTop15'),{
    type:'bar',
    data:{labels:rows.map(r=>'['+r.c+'] '+r.cat.replace(/^[0-9]+ - /,'').replace(/^[0-9]+-/,'').slice(0,28)),
...
function buildMatrixCharts(){
  ...
  charts.scatter=new Chart(document.getElementById('chartScatter'),{
    type:'scatter',
    data:{datasets:[{data:sc.map(r=>({x:r.x,y:r.y,cat:r.cat,c:r.c,role:r.role})),
      backgroundColor:sc.map(r=>(ROLE_COLOR[r.role]||'#6b7280')),pointRadius:4,pointHoverRadius:6}]},
  ...
  charts.share=new Chart(document.getElementById('chartShareDelta'),{
    ...
    data:{labels:seasonal.map(r=>'['+r.c+'] '+r.cat.slice(0,25)),
      datasets:[{label:'Share Delta pp',data:seasonal.map(r=>r.spd),
        backgroundColor:seasonal.map(r=>r.spd>=0?'#16a34a':'#dc2626'),borderRadius:3}]},
    options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,
      plugins:{legend:{display:false}},scales:{x:{title:{display:true,text:'Delta pp'}}}}
  });
  // Post-Easter behavior charts
  const pd=D.post_dist;
  charts.postBeh=new Chart(document.getElementById('chartPostBehavior'),{
    type:'doughnut',
    data:{labels:Object.keys(pd),datasets:[{data:Object.values(pd),backgroundColor:Object.keys(pd).map(k=>POST_COLOR[k]||'#9ca3af'),borderWidth:2}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom'}}}
  });
