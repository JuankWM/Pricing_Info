-- Category-level Easter analysis + post-Easter price variation
-- Extends original analysis with post-Easter weeks (202512-202517)
WITH src AS (
  SELECT
    COUNTRY_CODE, DEPT_CAT_NBR_DESC_COMB, SBU,
    WM_YEAR_WEEK_NBR,
  ...
  WHERE WM_YEAR_WEEK_NBR IN (
    ...
    202510,202511,
    202512,202513,202514,202515,202516,202517
  )
  GROUP BY 1,2,3,4
),
base AS (
  SELECT COUNTRY_CODE, DEPT_CAT_NBR_DESC_COMB, SBU,
...
preweek AS (
  SELECT COUNTRY_CODE, DEPT_CAT_NBR_DESC_COMB, SBU,
    SUM(wm_units)  AS pw_wm_units,
    SUM(tot_units) AS pw_tot_units,
    SUM(wm_sales)  AS pw_wm_sales,
    SUM(tot_sales) AS pw_tot_sales
  FROM src WHERE WM_YEAR_WEEK_NBR = 202510
  GROUP BY 1,2,3
),
easter AS (
  SELECT COUNTRY_CODE, DEPT_CAT_NBR_DESC_COMB, SBU,
...
posteaster AS (
  SELECT COUNTRY_CODE, DEPT_CAT_NBR_DESC_COMB, SBU,
    COUNT(DISTINCT WM_YEAR_WEEK_NBR) AS post_n,
    SUM(wm_units)  AS post_wm_units,
    SUM(mkt_units) AS post_mkt_units,
    SUM(wm_sales)  AS post_wm_sales,
    SUM(mkt_sales) AS post_mkt_sales
  FROM src WHERE WM_YEAR_WEEK_NBR BETWEEN 202512 AND 202517
  GROUP BY 1,2,3
)
SELECT
  e.COUNTRY_CODE,
  ...
  ROUND(b.base_tot_sales / b.base_n, 2) AS avg_base_tot_sales,
  -- Lifts
  ...
  -- Pre-week
  pw.pw_wm_units  AS preweek_wm_units,
  pw.pw_tot_units AS preweek_tot_units,
  -- Post-Easter price (avg of 6 post-Easter weeks)
  ROUND(SAFE_DIVIDE(p.post_wm_sales  / p.post_n, p.post_wm_units  / p.post_n), 4) AS avg_post_wm_price,
  ROUND(SAFE_DIVIDE(p.post_mkt_sales / p.post_n, p.post_mkt_units / p.post_n), 4) AS avg_post_mkt_amp,
  ROUND(p.post_wm_units  / p.post_n, 2) AS avg_post_wm_units,
  ROUND(p.post_mkt_units / p.post_n, 2) AS avg_post_mkt_units
FROM easter e
JOIN base b USING (COUNTRY_CODE, DEPT_CAT_NBR_DESC_COMB, SBU)
LEFT JOIN preweek pw USING (COUNTRY_CODE, DEPT_CAT_NBR_DESC_COMB, SBU)
LEFT JOIN posteaster p USING (COUNTRY_CODE, DEPT_CAT_NBR_DESC_COMB, SBU)
